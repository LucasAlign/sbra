...
export async function readDirectory(db: Database, personId: string, communityId: string, after?: string) {
  ...
  const [allowed] = await db.select({ id: s.communities.id }).from(s.communities)
  ...
  const rows = await db.select({ id: s.organizations.id, name: s.organizations.name,
...
export async function editOrganizationDescription(db: Database, personId: string, organizationId: string, description: string) {
  ...
  const changed = await db.update(s.organizations).set({ description: description.trim() })
